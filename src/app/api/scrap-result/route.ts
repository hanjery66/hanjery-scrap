...
async function getBrowserInstance() {
    try {
        if (!browser || browser.connected) {
            ...
            browser = await puppeteer.launch(env.NODE_ENV === "production" ? {
                headless: "shell",
...
export async function POST(request: Request) {
    ...
    try {
        ...
        const formattedDate = moment(date).format(isVn ? "DD-MM-YYYY" : "YYYY-MM-DD");
        ...
        if (isVn) {
            let column = 0;
            switch (_column) {
                case "V1": column = 1; break;
                case "V2": column = 2; break;
                case "V3": column = 3; break;
            }

            if (result_url.includes("minhngoc")) {
                const targetUrl = `${result_url.replace(".html", "")}/${formattedDate}.html`;
                ...
                lotteryResult = await currentPage.evaluate((col) => {
                    ...
                    const divs = results?.querySelectorAll('td[class^="giai"] div') ?? [];
                    return Array.from(divs).map(div => div.innerHTML).join("\n");
                }, column);

            } else {
                await currentPage.goto(
                    `https://dscgk04.82.112.235.163.sslip.io/?ket_qua_xo_so=${formattedDate}`,
                    { waitUntil: "networkidle0", timeout: 30000 }
                );

                await currentPage.waitForSelector(".js-layout-content", { timeout: 30000 });

                lotteryResult = await currentPage.evaluate((col, title) => {
                    ...
                    resultBoxes.forEach((el) => {
                        const resultTitle = el.querySelector(".js-header-title")?.innerHTML ?? "";
                        if (resultTitle.includes(title)) {
                            const resultRows = el.querySelectorAll(".js-table-tbody .js-table-row");
                            resultRows.forEach((resultRow, rowIndex) => {
                                if (rowIndex > 0) {
                                    const cells = resultRow.querySelectorAll(".js-table-cell");
                                    const rowCells = cells[col + 1]?.querySelector(".js-row")?.querySelectorAll(".js-col") ?? [];
            ...
            await currentPage.waitForSelector('input.form-control[name="date"]', { timeout: 30000 });
            await currentPage.$eval('input.form-control[name="date"]', (input) => (input.value = ""));
            ...
            lotteryResult = await currentPage.evaluate((url) => {
                ...
                rows.forEach((row) => {
                    ...
                    cells.forEach((cell, index) => {
                        if (index > 0) {
                            map.set(shiftName, [...(map.get(shiftName) ?? []), cell.innerHTML]);
                        } else {
